...
public class AdmissionControlStats implements ToXContentFragment, Writeable {
    ...
    List<BaseAdmissionControllerStats> statsList;
    public AdmissionControlStats(List<BaseAdmissionControllerStats> statsList){
        this.statsList = statsList;
    }

    public AdmissionControlStats(StreamInput in) throws IOException {
        if (in.getVersion().onOrAfter(Version.V_3_0_0)) {
            this.statsList = in.readNamedWriteableList(BaseAdmissionControllerStats.class);
        } else {
            this.statsList = null;
        }
    }
